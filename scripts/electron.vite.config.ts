...
const copyClientViewerStaticFiles = () => {
	return {
		...
		async writeBundle() {
			const sourceDir = resolve(projectRoot, 'src/client-viewer/dist');
			const destDir = resolve(projectRoot, 'out/client-viewer');
...
const copySimplePeerMinJsStaticFiles = () => {
	return {
		...
		async writeBundle() {
			...
			const destDir = resolve(projectRoot, 'out/renderer/assets');
		...
		build: {
			...
			rollupOptions: {
				input: {
					...
					helperRenderer: resolve(projectRoot, 'src/preload/index.ts'),
					// webview: resolve(projectRoot, 'src/preload/webview.js')
				},
			},
		},
		plugins: [externalizeDepsPlugin(), bytecodePlugin()],
